...
*/
  ...
  if (savedSettingSenderid==1) {
    ...
    mess = "There is no stored senderid, using default value";
...
*/
  ...
  char buf[60];
  sprintf(buf, "%s @ IP:%d.%d.%d.%d SSID: %s", APPNAME, WiFi.localIP()[0], WiFi.localIP()[1], WiFi.localIP()[2], WiFi.localIP()[3], MYSSID );
  Serial.println(buf);
  /*
  root["status"] = buf;
  root.printTo((char*)msg, root.measureLength() + 1);
